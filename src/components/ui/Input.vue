...
<template>
  <div class="form-float" :class="[divClass, { 'form-float--error': error }]">
    <input :id="inputId" :type="type" :placeholder="placeholder" :value="modelValue" :disabled="disabled" :required="required" :autocomplete="autocomplete" :class="[inputClass, { 'form-float__input--error': error }]" @input="$emit('update:modelValue', $event.target.value)" @focus="$emit('focus', $event)" @blur="$emit('blur', $event)" />
